function run_getSignals_2DLand_fixed1stFrame(folder,movingWindowWidth,frameRate,movie2Use)
...
for movingWinCnt = 0:numMovingWin
    ...
    matName = sprintf('bvpSignal_2DLand_fixed1stFrame_%02dsec-%02dsec.mat',startTime,endTime);%+1/frameRate);
    if ~exist(fullfile(matFolder,matName),'file')
        [bvpCandidates,~,time] = getSignals_2DLand_fixed1stFrame(videoFileName,startTime,endTime,angleThres);
        savehelper(bvpCandidates,frameRate,time,matFolder,matName)
    end
end
end
function savehelper(bvpCandidates,frameRate,time,matFolder,matName)
save(fullfile(matFolder,matName),'bvpCandidates','frameRate','time')
end