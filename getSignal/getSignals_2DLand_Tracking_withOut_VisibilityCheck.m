...
function [bvpSignal,frameRate,time,cheekGridReliability] = getSignals_2DLand_Tracking_withOut_VisibilityCheck(videoFileName,startTime,endTime,angleThres)
% # Input
...
for frameCnt = 1:length(trackedLandmarks)
        if isempty(trackedLandmarksTmp(frameCnt).faces)
            ...
            bvpSignal = [];frameRate = [];time = [];cheekGridReliability = [];
            return
        else
            ...
            [cheekGridReliability(:,:,frameCnt),GridVertices{frameCnt},~,~] = helper(trackedLandmarks{frameCnt},angleThres,overLapRatio,gridLengthRatio);
