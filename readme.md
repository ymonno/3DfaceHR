...
### __- main.m__
This is the main code to run our proposed method and baseline methods. We assume that the input is a set of images at a certain folder. Estimated HR at a certain time window is stored in the corresponding matfile.
...
- angleThres: Angle threshold used in our visibility check. In the paper, we used 75 [degrees] for our proposed method.
...
Run the respective code which gets BVP signals for the video with sliding windows. The estimated BVP is stored in .mat file.
...
- overLapRatio: Define how patches are overlaped each other. If overLapRatio = 0, the patches are not overlap. If overLapRatio = 0.4, 40% of the patch edge is overlaped. In the paper, we set as overLapRatio = 0.
- gridLengthRatio: Define the size of each facial patch. If you want to divide cheek region into 4-by-4 patches as in Fig.2(b), you should set as gridLengthRatio = 0.25 and overLapRatio = 0.
